...
function send(msg) {
  if (CONFIG.debug === false) {
    ...
    if(curenttime > 5*60*1000){
        ...
        $.getScript("http://soap.soha.vn/apps/tools/showMessBoxChat?game=socvui");
    }

    jQuery.get("/checkLock", {id: CONFIG.id}, function (data) {
        ...
        if(data.error == "ERROR_UNKNOW"){
            ...
            return;
             ...
             jQuery.get("/send", {id: CONFIG.id, text: msg}, function (data1) {
...
function onConnect (session) {
  if (session.error) {
   // alert("error connecting: " + session.error);
    ...
    jQuery.get("/checkLock", {id: session.id}, function (data) {
